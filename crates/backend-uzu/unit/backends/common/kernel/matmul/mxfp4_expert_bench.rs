...
use crate::{
    ...
    backends::{
        ...
        metal::{DEFAULT_GEMV_MAX_BATCH, Int8Execution, Metal, MetalContext, ResidentInt8ExpertTensorOpsDispatch},
    },
...
fn bench_resident_exact_int8_w2(
    ...
    let mut tensorops = ResidentInt8ExpertTensorOpsDispatch::new(DataType::F32, DataType::BF16, DataType::BF16);
...
fn bench_mxfp4_expert_prefill_production(c: &mut Criterion) {
    ...
    let gate = GatedActMul::<Metal>::full_precision(
        context,
        crate::data_type::DataType::F32,
        true,
        false,
        GatedActMulSettings {
            activation_alpha: Some(1.702),
            gate_clipping: ClippingBounds::from(Some((Some(-7.0), Some(7.0)))),
            value_clipping: ClippingBounds::from(Some((Some(-7.0), Some(7.0)))),
        },
    )
    .expect("gate kernel");
