...
use crate::{
    backends::{
        common::{
            Allocation, AllocationPool, AllocationType, Allocator, Backend, Context, DeviceCapabilities, Int8Execution,
        },
        metal::{
...
impl MetalContext {
    ...
    pub fn supports_int8_tensorops(&self) -> bool {
        ...
        *self.int8_tensorops.get_or_init(|| {
            ResidentInt8ExpertTensorOpsMetalKernel::new(self, DataType::F32, DataType::F32, DataType::F32, false)
                .is_ok()
        })
    }

    /// Hardware implementation when available, otherwise the portable numerical equivalent.
    pub fn int8_execution(&self) -> Int8Execution {
        if Int8Execution::emulation_requested() || !self.supports_int8_tensorops() {
            return Int8Execution::Emulated;
        }
