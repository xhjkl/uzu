...
use crate::{
    backends::{
        ...
        metal::{Metal, context::MetalContext, error::MetalError, kernel::ResidentInt8ExpertTensorOpsMetalKernel},
    },
    data_type::DataType,
};

/// Resident group-32 signed-INT8 expert projection on M5 TensorOps.
#[allow(dead_code)]
pub(crate) struct ResidentInt8ExpertTensorOpsDispatch {
    bias_data_type: DataType,
    output_data_type: DataType,
    pipelines: HashMap<bool, ResidentInt8ExpertTensorOpsMetalKernel>,
}
...
impl ResidentInt8ExpertTensorOpsDispatch {
    ...
    fn get_or_create(
        ...
        match self.pipelines.entry(expert_bias) {
    ...
    pub(crate) fn encode(
        ...
        require(
            "weight scales",
            weight_scales.size(),
            checked_size(&[experts, output_size, group_count, size_of::<f32>()]),
        )?;
        ...
        let pipeline = self.get_or_create(encoder.context(), expert_biases.is_some())?;
        pipeline.encode(
            weight_codes,
            weight_scales,
            activation_codes,
            ...
            expert_count.get(),
            matches!(input, ExpertInput::Routes),
            output_tile_count,
            encoder,
        );
        Ok(())
